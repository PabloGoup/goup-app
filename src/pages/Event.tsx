...
function LoadingButton({
  ...
  return (
    <button
      className={`inline-flex items-center justify-center rounded-md px-5 py-2.5 text-sm font-semibold transition disabled:opacity-50 bg-[#8e2afc] hover:bg-[#7b1fe0] ${className}`}
      disabled={loading}
      {...rest}
    >
      {loading ? "…" : children}
...
function EventFormSinglePage() {
  ...
  const fieldCls =
    "w-full bg-white/5  placeholder-white/40 border /10 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#8e2afc]";
      ...
      <FormProvider {...methods}>
        <form onSubmit={onSubmit} noValidate className="max-w-5xl mx-auto space-y-8">
          ...
          <Section title="Concepto & experiencia">
            ...
            <RHFCheckboxGroup name="generos" label="Géneros musicales *" options={[
          ...
          <div className="sticky bottom-0 bg-black/10 backdrop-blur border-t /5 py-3">
            <div className="max-w-5xl mx-auto flex flex-col sm:flex-row gap-2 justify-center">
              <LoadingButton type="submit" className="w-full sm:w-auto">Crear evento</LoadingButton>
            </div>
          </div>
        </form>
      </FormProvider>
